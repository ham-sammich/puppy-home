...
pub fn render(ui: &mut egui::Ui, sup: &Supervisor, actions: &mut Vec<ShellAction>) {
    ...
    if !waiting.is_empty() {
        ...
        egui::Frame::group(ui.style()).fill(egui::Color32::from_rgb(40, 28, 42)).show(ui, |ui| {
        ...
        egui::Grid::new("dashboard-grid")
            ...
            .show(ui, |ui| {
                ...
                for ws in sup.iter() {
                    ...
                    ui.painter().circle_filled(rect.center(), 5.0, ws.status.color());
                    ...
                    // agent / model
                    ui.vertical(|ui| {
                        ui.label(if ws.agent.is_empty() { "—" } else { &ws.agent });
                        ui.label(
                            egui::RichText::new(if ws.model.is_empty() { "—" } else { &ws.model })
                                .weak()
                                .small(),
                        );
                    });
                    ...
                    ui.label(if waiting { state_text.strong() } else { state_text });

                    // activity: elapsed + tool count
                    let mut activity = match ws.turn_started {
                        Some(start) => format!("{}s", now.saturating_duration_since(start).as_secs()),
                        None => format!(
                            "{} ago",
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui.button(diff_label).on_hover_text("File changes").clicked() {
                    ...
                    for sa in &ws.sub_agents {
                        ...
                        ui.label(
                            egui::RichText::new(format!("↳ {}", sa.agent_name)).small(),
                        );
