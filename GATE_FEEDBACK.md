...
| F3 | App cuts off items at the bottom of (some) views; can't scroll them into view, must resize the window | global layout | DONE: the offender was the DASHBOARD grid. The dash-scroll container was correct, but the screen `body` slot was rendered `size_full` inside the top-level toolbar+strip+body flex-column, so body demanded 100% height ON TOP of the toolbar/strip -> its scroll viewport's bottom fell below the window and the lower cards couldn't be scrolled into view. Fix: wrap the body slot in flex_1 + min_h_0 so it takes the REMAINING height (applies to chat/browser too). |
...
| F7 | New workspace card: screen flashes on add, and the card appends to the END of the list while the eye expects top-left | dashboard | DONE: root cause was status-rank sorting. A cold spawn boots as Starting (was ranked with the live band = near top) then settles to Idle (rank 3 = tail) -> the jump. Fix: rank Starting WITH Idle (a cold spawn has no work, precedent: the neutral border already groups them) ; per Jacob the new card should land at the END and STAY there (not top). Keeping Starting ranked with Idle is the whole fix: a spawn now appears at the tail of the resting group and stays put instead of bouncing top->end as it boots. (Dropped the newest-first reverse.) Entrance fade is keyed by (card-in, id) so it plays once and does not replay on reorder. |
| F8 | Agent creator opens a NEW WORKSPACE; should stay localized inside the agent modal ("create with agent creator" button) | agents manager | DONE: "Create with Agent Creator" now runs the agent-creator chat on a HIDDEN ephemeral session (new Workspace.ephemeral flag; supervisor.iter_visible() excludes it from the dashboard fleet, fleet stats, session save, headline puppy, serving-ws). Rendered as its OWN modal (transcript + composer, priority 240, occluding scrim) floating over the agents manager -- no dashboard card. "Done" (AgentCreatorClose) kills the sidecar (drop -> child killed, no orphan) and refreshes the agents list. |
| F9 | Agent creator: focus is a weird shifted view | agents manager | LIKELY RESOLVED by F8: the creator is now a contained modal (not a full chat-screen navigation), and pending_focus auto-focuses the modal's composer. Jacob to confirm the focus feels right in the new modal. |
...
| F11 | Profile pfp is emoji-only; want real photo uploads | profile/identity | DONE (local): avatar value is now EITHER an emoji OR a photo path. "Choose photo..." in the avatars panel -> native file picker -> store_photo() copies into <data-dir>/avatars/{user|puppy}-{ts}.{ext} (timestamped to dodge the image cache). Rendered via avatars::boxed/fill_parent (img cover-fit, clipped) in: transcript rows, empty-state, dashboard lede, toolbar identity chip, picker preview. Inline-text spots (ask/sessions) fall back to the default emoji via avatars::inline so paths never leak. DEFERRED: den transport (relay has no avatar slot -- needs a protocol extension + 2 devices). |
...
| P4 | Whistle action uses a horn emoji (postal horn U+1F4EF); want a whistle | DONE: shipped a real whistle. Unicode has no whistle emoji, so added assets/icons/whistle.svg (monochrome silhouette: mouthpiece + chamber + loop stem) served through the gpui AssetSource (new ICONS list) and rendered via gpui::svg() tinted to text color. New widgets::icon_btn(t, icon, label) helper (DRY mirror of btn). Dashboard "Whistle" button now uses it instead of the U+1F4E3 megaphone. |
